...
# Cancel all shard processing jobs
# Usage: ./cancel_all_jobs.sh
#
# This will cancel all jobs with names starting with "mls"

set -euo pipefail

echo "============================================"
echo "Cancel All Shard Jobs"
echo "============================================"
echo ""

# Get list of all mls jobs
JOBS=$(squeue -u "$USER" -h -o "%i %j" 2>/dev/null | grep "mls" || echo "")

if [ -z "$JOBS" ]; then
    echo "No mls jobs found in queue."
    exit 0
fi
...
REMAINING=$(squeue -u "$USER" -h -o "%j" 2>/dev/null | grep -c "^mls" || echo "0")
