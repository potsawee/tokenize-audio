...
HF_SOURCE_REPO = "fixie-ai/common_voice_17_0"
...
def download_shard(split_name: str, shard_id: str, cache_dir: str = "./cache") -> str:
    ...
    Args:
        split_name: Split name like 'en', 'fr', 'de', ...
        shard_id: Shard ID like 'validated-00087-of-00138'
...
def process_shard(
    ...
    for batch_start in tqdm(range(0, len(dataset), batch_size), desc=f"Encoding {shard_id}"):
        ...
        try:
            ...
            logger.warning(f"Skipping batch {batch_start}-{batch_end} due to error: {e}")
            continue
        ...
        for i in range(batch_end - batch_start):
            ...
            transcript = batch["sentence"][i]
            file_id = batch["path"][i]
...
def main():
    parser = argparse.ArgumentParser(description="Process Common Voice shards with Mimi encoding")
    parser.add_argument("--shard-id", type=str, help="Single shard ID to process (e.g., validated-00087-of-00138)")
    parser.add_argument("--split-name", type=str, required=True, help="Split name like 'en', 'fr', 'de', ...")
...
if __name__ == "__main__":
    main()
    # Example usage:
    # python process_common_voice.py --split-name en --shard-id validated-00087-of-00138 --hf-repo-id potsawee/commonvoice17-mm-pretrain --output-dir /sphinx/u/salt-checkpoints/commonvoice17/output --cache-dir /sphinx/u/salt-checkpoints/commonvoice17/cache
    # python process_common_voice.py --split-name en --shard-id-list shard_list.txt --hf-repo-id potsawee/commonvoice17-mm-pretrain --output-dir /sphinx/u/salt-checkpoints/commonvoice17/output --cache-dir /sphinx/u/salt-checkpoints/commonvoice17/cache
