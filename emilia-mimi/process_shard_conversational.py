...
class EmiliaShardProcessor:
    ...
    def process(self):
        ...
        for doc, audio_files_in_doc in documents.items():
            doc_text_for_type1 = ""
            list_of_speaker_ids = []
            speaker_id_mapping = {}  # Map speaker_id to integer based on first appearance order
            for audio_file in audio_files_in_doc:
                json_file_path = self.audio_extract_dir / f"{audio_file}.json"
                with open(json_file_path, 'r') as f:
                    metadata = json.load(f)
                speaker_id = metadata['speaker']
                assert "SPEAKER_" in speaker_id, f"Speaker ID {speaker_id} does not start with 'SPEAKER_'"
                if speaker_id not in speaker_id_mapping:
                    speaker_id_mapping[speaker_id] = len(speaker_id_mapping)
                speaker_id_int = speaker_id_mapping[speaker_id]

                audio_str = dict_of_audio_str[audio_file]["audio_str"].strip()
                transcript = dict_of_audio_str[audio_file]["transcript"].strip()
                doc_text_for_type1 += f"<|text_start|>[{str(speaker_id_int)}]{transcript}<|text_end|><|audio_start|>{audio_str}<|audio_end|>"
                list_of_speaker_ids.append(speaker_id_int)
            ...
            pretraining_data.append({
                ...
                "speaker_count": len(set(list_of_speaker_ids))
            })
        ...
        dataset = Dataset.from_dict({
            ...
            "speaker_ids": [item["speaker_ids"] for item in pretraining_data],
            "speaker_count": [item["speaker_count"] for item in pretraining_data],
        })
...
if __name__ == "__main__":
    main()
    # Emilia-YODAS-EN
    # usage: python process_shard_conversational.py --split Emilia-YODAS --shard-id EN-B000000 --hf-repo-id potsawee/emilia-mm-conversational --work-dir /sphinx/u/salt-checkpoints/emilia-mm-conversational/work-yodas --output-dir /sphinx/u/salt-checkpoints/emilia-mm-conversational/output-yodas --progress-dir /sphinx/u/salt-checkpoints/emilia-mm-conversational/progress-yodas --device cuda --batch-size 64
