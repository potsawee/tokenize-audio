...
# Usage: 
#   ./submit_all_shards_yodas.sh [max_concurrent_jobs] [shard_list_file]
#   ./submit_all_shards_yodas.sh [shard_list_file]  (defaults to 100 max jobs)
# Examples: 
#   ./submit_all_shards_yodas.sh 100 file_lists/en_yodas.txt
#   ./submit_all_shards_yodas.sh file_lists/en_yodas.txt
#   ./submit_all_shards_yodas.sh 50
#
...
echo "============================================"
echo "Batch YODAS Job Submission with Concurrency Limit"
echo "============================================"
...
# Function to count current YODAS jobs in queue (job names like ye_1361, yd_5, etc.)
count_current_jobs() {
...
# Get currently running/pending YODAS jobs (job names like ye_1361, yd_5, etc.)
...
for SHARD_ID in "${SHARDS_TO_SUBMIT[@]}"; do
    ...
    # Run submit_shard_yodas.sh and check if it succeeds
    if ./submit_shard_yodas.sh "$SHARD_ID" > /dev/null 2>&1; then
...
echo "  python monitor_progress.py --progress-dir /sphinx/u/salt-checkpoints/emilia-mm-pretrain/progress-yodas --work-dir /sphinx/u/salt-checkpoints/emilia-mm-pretrain/work-yodas"
