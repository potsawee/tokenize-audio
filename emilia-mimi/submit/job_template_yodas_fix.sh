...
#SBATCH --output=/sphinx/u/salt-checkpoints/emilia-mm-pretrain-fix/logs-yodas/default.log
#SBATCH --error=/sphinx/u/salt-checkpoints/emilia-mm-pretrain-fix/logs-yodas/default.log
...
if [ -z "$SHARD_ID" ]; then
    ...
    echo "Usage: sbatch submit/job_template_yodas_fix.sh <shard_id>"
    echo "Example: sbatch submit/job_template_yodas_fix.sh EN-B001361"
    exit 1
fi
...
WORK_DIR="/sphinx/u/salt-checkpoints/emilia-mm-pretrain-fix/cache-dir-yodas"
...
python -u process_shard_fix.py \
    --split Emilia-YODAS \
    --shard-id ${SHARD_ID} \
    --work-dir /sphinx/u/salt-checkpoints/emilia-mm-pretrain-fix/work-dir-yodas \
    --cache-dir /sphinx/u/salt-checkpoints/emilia-mm-pretrain-fix/cache-dir-yodas