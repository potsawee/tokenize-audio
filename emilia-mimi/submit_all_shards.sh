...
# Usage: 
#   ./submit_all_shards.sh [max_concurrent_jobs] [shard_list_file]
#   ./submit_all_shards.sh [shard_list_file]  (defaults to 100 max jobs)
# Examples: 
#   ./submit_all_shards.sh 100 file_lists/en.txt
#   ./submit_all_shards.sh file_lists/en_temp.txt
#   ./submit_all_shards.sh 50
#
# This script reads from file_lists/en.txt (or specified file) and submits jobs
...
if [ -f "${1:-}" ]; then
    ...
    SHARD_FILES_LIST="${2:-file_lists/en.txt}"
else
...
# Function to count current jobs in queue (job names like en_0, de_5, etc.)
count_current_jobs() {
    local count=$(squeue -u "$USER" -h -o "%j" 2>/dev/null | grep -cE "^[a-z]{2}_[0-9]+" 2>/dev/null || true)
    echo "${count:-0}"
}
...
# Get currently running/pending jobs (job names like en_0, de_5, etc.)
echo "Checking which shards are already in queue..."
CURRENT_JOBS=$(squeue -u "$USER" -h -o "%j" 2>/dev/null | grep -E "^[a-z]{2}_[0-9]+" || echo "")
...
for SHARD_ID in "${SHARD_IDS[@]}"; do
    # Create job name (EN-B000000 -> en_0)
...
echo "  squeue -u \$USER"
echo "  watch -n 10 'squeue -u \$USER | grep -E \"^[a-z]{2}_[0-9]+\"'"
echo ""
echo "Check progress with:"
echo "  python monitor_progress.py --progress-dir /sphinx/u/salt-checkpoints/emilia-mm-pretrain/progress --work-dir /sphinx/u/salt-checkpoints/emilia-mm-pretrain/work"
