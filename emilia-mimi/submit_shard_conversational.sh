...
if [ -z "$SHARD_ID" ]; then
    ...
    echo "Usage: ./submit_shard_conversational.sh <shard_id>"
    echo "Example: ./submit_shard_conversational.sh EN-B001361"
    exit 1
fi
...
# Set log directory
LOG_DIR="/sphinx/u/salt-checkpoints/emilia-mm-conversational/logs-yodas"
...
sbatch \
    ...
    --output="${LOG_FILE}" \
    --error="${LOG_FILE}" \
    submit/job_template_conversational.sh "${SHARD_ID}"
SBATCH_EXIT=$?
