...
class Trainer:
    ...
    def _iterate_and_convert(self, codes_files: List[str]) -> Iterator[str]:
        for codes_file in codes_files:
            codes_data = np.load(codes_file, allow_pickle=True)
            
            # Check if this is the new consolidated format (list of arrays) or old format (single array)
            if isinstance(codes_data, np.ndarray) and codes_data.dtype == object and len(codes_data.shape) == 0:
                # New format: file contains a list of code arrays
                codes_list = codes_data.item()
                if not isinstance(codes_list, list):
                    codes_list = [codes_list]
            elif isinstance(codes_data, np.ndarray) and codes_data.dtype == object and len(codes_data.shape) == 1:
                # New format: array of arrays
                codes_list = list(codes_data)
            else:
                # Old format: single array, wrap in list for uniform processing
